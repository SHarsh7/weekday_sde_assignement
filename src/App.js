...
function App() {
  ...
  useEffect(() => {
    ...
    const filteredData = jobs.filter(job => {
      ...
      const matchesRole =
        !selectedRole ||
        job.jobRole.toLowerCase() === selectedRole.toLowerCase();
      const matchesExperience =
        !selectedExperienceRange ||
        (job.minExp >= selectedExperienceRange[0] &&
          job.maxExp <= selectedExperienceRange[1]);
      const matchesSalary =
        !selectedSalaryRange ||
        (job.minJdSalary >= selectedSalaryRange[0] &&
          job.maxJdSalary <= selectedSalaryRange[1]);
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6} md={4} lg={3}>
              <FormControl size='small' variant='filled' fullWidth>
                ...
                <Select
                  value={
                    selectedSalaryRange ? selectedSalaryRange.join(",") : ""
                  }
            ...
            <Grid item xs={5}>
              <Button
                fullWidth
                variant='contained'
                style={{ backgroundColor: "#5CE7BD", color: "black" }}
                onClick={handleReset}>
                Reset Filters
              </Button>
            </Grid>
